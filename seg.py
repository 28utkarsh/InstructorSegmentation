...
class DeepLabModel(object):
    ...
    """
        ...
        for contour in contours:
            c_areas.append(cv2.contourArea(contour))
        ...
        max_contour_area = contours[np.array([c_areas]).argmax()]
         for i, area in enumerate(c_areas):
            if area < (seg_height * seg_width * 0.2) or cv2.pointPolygonTest(max_contour_area,(centroids[i][0], centroids[i][1]),True) > 0:
        ...
        if sys.argv[4] == '--video':
            ...
            while cap.isOpened():
                ...
                if ret is True and i%2 != 0:
                    ...
                    #seg_map = cv2.morphologyEx(seg_map, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 10)
                    seg_map = cv2.erode(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)), iterations = 5)
                    seg_map = cv2.dilate(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)), iterations = 1)
                    # seg_map = cv2.erode(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5)), iterations = 2)
                    # seg_map = cv2.dilate(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 1)
                    # seg_map = cv2.erode(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5)), iterations = 2)
                    # seg_map = cv2.dilate(seg_map, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 1)
                    to_write_frame = drawSegment_cv(resized_im, seg_map, vid = True)
                    out.write(to_write_frame)
                    cv2.imshow('frame',to_write_frame)
                    #cv2.imshow('seg_frame', seg_map)
