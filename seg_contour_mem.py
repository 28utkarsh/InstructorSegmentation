...
class DeepLabModel(object):
    ...
    def __init__(self, tarball_path):
        ...
        self.prev_seg_maps = collections.deque([], maxlen=6)
    ...
    """
        ...
        #contours, hierarchy = cv2.findContours(seg_map, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        ...
        seg_map = np.array(self.prev_seg_maps).mean(axis=0)
...
def run_visualization(filepath):
    ...
    try:
        ...
        if sys.argv[4] == '--video':
            ...
            while cap.isOpened():
                ret, frame = cap.read()
                if ret is True and first_frame:
